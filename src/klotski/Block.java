...
class Block extends JPanel implements MouseListener, MouseMotionListener {
  ...
  BufferedImage getImg(String input) {
    ...
    try {
      myImage = ImageIO.read(Main.class.getResourceAsStream(type));
  ...
  public void mouseDragged(MouseEvent e) {
    ...
    // When movement is < 5 pixels then do nothing
  ...
  boolean movable(int dx, int dy) {
    ...
    if (dx < 0) {
      newLocationX = (int) xFloor;
    }
    if (dx > 0) {
      newLocationX = (int) xCeil;
    }
    if (dy < 0) {
      newLocationY = (int) yFloor;
    }
    if (dy > 0) {
      newLocationY = (int) yCeil;
    }

    // Not allow block move over board
    if (newLocationX < 0 || newLocationY < 0) {
      return false;
    }
    if (newLocationX + column - 1 >= Board.W || newLocationY + row - 1 >= Board.H) {
      return false;
    }
    ...
    for (int i = 0; i < column; i++) {
      for (int j = 0; j < row; j++) {
        if (BlockList.occupied[newLocationX + i][newLocationY + j]) {
          return false;
        }
      }
    }
